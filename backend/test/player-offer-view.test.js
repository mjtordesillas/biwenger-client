...
test('shapes a {offer, player} pair, carrying the offer amount and bidder through', () => {
  const view = toPlayerOfferView({
    offer: { amount: 300000, from: { id: 12, name: 'Rival FC' }, to: { id: 42 }, requestedPlayers: [16738], status: 'waiting', type: 'purchase' },
...
test('sets bidder to null when the offer\'s `from` is null (the observed case so far)', () => {
  const view = toPlayerOfferView({
    offer: { amount: 200000, from: null, to: { id: 42 }, requestedPlayers: [10650], status: 'waiting', type: 'purchase' },
