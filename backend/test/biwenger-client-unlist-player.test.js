...
test('unlistPlayer authenticates and DELETEs the market endpoint with the player id as a query param', async () => {
  const calls = []
  ...
  await client.unlistPlayer({ email: 'test@example.com', password: 'secret', playerId: 37817 })

  assert.deepEqual(calls[2], {
    url: 'https://example.test/api/v2/market?player=37817',
    options: {
      method: 'DELETE',
      headers: {
