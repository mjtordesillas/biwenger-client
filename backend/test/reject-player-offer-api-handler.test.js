...
import { createRejectPlayerOfferApiHandler } from '../src/reject-player-offer-api-handler.js'

test('forwards the path offer id with configured credentials and returns 200', async () => {
  let request
  const handler = createRejectPlayerOfferApiHandler({
    biwengerClient: { rejectOffer: async (value) => { request = value } },
...
test('returns a sanitized 502 for an invalid id or upstream failure', async () => {
  const handler = createRejectPlayerOfferApiHandler({
    biwengerClient: { rejectOffer: async () => { throw new Error('token=super-secret-leak') } },
  })
