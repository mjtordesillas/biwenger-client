...
test('forwards the body playerId and amount with configured credentials and returns 200', async () => {
  let request
  ...
  const response = await handler({ body: JSON.stringify({ playerId: 24956, amount: 150000 }) })
...
test('returns a sanitized 502 for an invalid id, invalid amount, malformed body, or upstream failure', async () => {
  ...
  const invalidId = await handler({ body: JSON.stringify({ playerId: 'nope', amount: 150000 }) })
  const invalidAmount = await handler({ body: JSON.stringify({ playerId: 1, amount: -5 }) })
  const malformedBody = await handler({ body: 'not-json' })
  const failed = await handler({ body: JSON.stringify({ playerId: 1, amount: 150000 }) })
