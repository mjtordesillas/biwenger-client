...
test('rejectOffer authenticates and PUTs the verified rejected status to the offer endpoint', async () => {
  const calls = []
  const client = createBiwengerClient({
    ...
    fetch: async (url, options = {}) => {
      ...
      return { ok: true, json: async () => ({ status: 200, data: { status: 'rejected' } }) }
    },
  })

  await client.rejectOffer({ email: 'test@example.com', password: 'secret', offerId: 3822815314 })
