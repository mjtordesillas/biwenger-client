...
// Private write proxy for DELETE /market/my-bids/{offerId}. Upstream
// details are deliberately collapsed so a response can never disclose
// credentials.
export const createRemoveBidApiHandler = (dependencies = {}) => {
  const {
  ...
  return async (event) => {
    try {
      ...
      await biwengerClient.removeBid({ ...credentials, offerId })
      return {
        statusCode: 200,
