...
class HttpSquadServiceTest {
    ...
    fun `squad parses the wrapped players array`() {
        runBlocking {
            server.enqueue(
                MockResponse().setBody(
                    """{"players":[{
                        ...
                        "signedAt":1786943457,"signedPrice":3850123,"draftedPrice":null,
            ...
            assertThat(player?.signedAt).isEqualTo(1786943457)
            assertThat(player?.signedPrice).isEqualTo(3850123)
            assertThat(player?.draftedPrice).isNull()
